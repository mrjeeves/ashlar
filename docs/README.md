...
| [`decisions/`](decisions/) | ADRs 0001–0017: what was decided, why, and what it cost — from the name itself to the stylesheet boundary, view reconciliation, semantic freedom and derivability, the origin-not-edge deployment posture, the proposed data layer (databases named in source, bound in deployment; kept off the loop's blocking path), the storage-scope cleanup (retire `synced`, add per-user `owned`), the shared design language + live showcase for the examples (with the `--port` run-time override), and the foreign boundary as a capability whose transport is bound in deployment (keyed by space name, so the refactors, the checker, and the manifest all see it). |
