...
| E006 | A4 | checker | shape mismatch: cause states the expected and actual shape — including pipe layers disagreeing in parameter/return shape (§4), a `return` literal that does not fit the shape its pipe property agreed on (ADR-0025), and `stack` return keys that are not state properties | mechanical edits where safe (`text(...)` wrap on mixed `+`, `!= none` on optional conditions); precise notes otherwise |
