...
**C9.** A change to the use graph that alters composition order is **reported**. Where a previous build's derived state is available, a source edit that resequences any part's layers produces a diagnostic naming the part and its order before and after. A deterministic behavioral change that no output mentions is a failure even though the result is reproducible: determinism says the same source gives the same program, and says nothing about whether the author was told the program changed.
---
...
**D5.** The number of round trips from "agent writes code" to "code is correct" is the measure of the compiler's quality. Every diagnostic that is a correction removes a round trip. The measurement covers the **whole** diagnostic corpus: the proportion carrying a machine-applicable fix is reported alongside the mean rounds-to-clean of those that do, because a mean computed only over the fixtures that already have fixes cannot detect a diagnostic that stopped having one.
**D6.** **A machine-applicable fix never changes what a name resolves to.** D2 requires that applying a fix produces source that compiles; D6 requires that it produces the *same program*. Where restoring the author's meaning is not derivable — an ambiguous name with two candidates and nothing to say which was meant — the diagnostic carries a note naming every candidate and no edits. A fix that compiles and silently means something else is worse than no fix, because it is applied without review and leaves nothing to review.
---
